...
export async function runInstall(source: string | undefined, options: InstallOptions): Promise<void> {
  ...
  try {
    ...
    const targetAgents = await resolveTargetAgents(options.agent, options.yes, detectedAgents);
    if (!targetAgents) {
